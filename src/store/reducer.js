...
const reducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case at.NEW_ACTIVITY_CHANGE:
            if (state.new_activity.status !== WEB.NONE) {
                return state;
            }
            return {
                ...state,
                new_activity: { ...state.new_activity, ...action.activity }
            };
        ...
        case at.NEW_ACTIVITY_TOGGLE_CONDITION:
            ...
            if (conditions.includes(action.toggle)) {
                return {
                    ...
                    new_activity: {
                        ...state.new_activity,
                        conditions: conditions.filter(c => c !== action.toggle)
                    }
                };
            } else {
                return {
                    ...
                    new_activity: {
                        ...state.new_activity,
                        conditions: [...conditions, action.toggle]
                    }
                };
            }
        ...
        case at.NEW_ACTIVITY_SEND:
            return {
                ...state,
                new_activity: { ...state.new_activity, status: WEB.REQUEST }
            };

        case at.NEW_ACTIVITY_SUCCESS:
            return {
                ...state,
                new_activity: { ...state.new_activity, status: WEB.SUCCESS }
            };
